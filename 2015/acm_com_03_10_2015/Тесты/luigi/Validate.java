/**
 * Created by Shovkoplyas on 01.10.2015.
 */
...
public class Validate {
    final int MAXN = 1_000_000_000;

    public void run() {
        StrictScanner inf = new StrictScanner(System.in);
        int A = inf.nextInt();
        ensureLimits(A, 1, MAXN, "Incorrect A");
        int B = inf.nextInt();
        ensureLimits(B, A, MAXN, "Incorrect B");
        int C = inf.nextInt();
        ensureLimits(C, 1, MAXN, "Incorrect C");
        int D = inf.nextInt();
        ensureLimits(D, C, MAXN, "Incorrect D");
        int k = inf.nextInt();
        ensureLimits(k, 1, MAXN, "Incorrect k");
        inf.nextLine();
        inf.close();
    }
