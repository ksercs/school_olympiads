...
my $lang = undef;
`g++ -O3 $source_name.cpp -o $source_name`;
if ($? == 0) {
	$lang = "cpp";
} else {
...
sub test {
    for my $test (glob shift) {
        ...
        if ($lang eq "cpp") {
            ...
            `bash -c '{ time timeout $TL java $source_name ; } 2>log.txt'`;
        }
        my $res = $?;
        open F, "<log.txt";
        my $time = "";
        ...
        $time = $1;
        if ($res == 0) {
            my $output = `java -Xmx256M -Xss64M -cp ../../../lib/testlib4j.jar:. ru.ifmo.testlib.CheckerFramework Check $test $problem.out $test.a`;
            if ($? == 0) {
