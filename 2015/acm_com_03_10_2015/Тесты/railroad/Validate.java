...
public class Validate {
    ...
    public static final int MAX_VAL = 1000_000_000;
    ...
    public void run() {
        ...
        int m = inf.nextInt();
        int x = inf.nextInt();
        ensureLimits(n, 1, N, "n isn't valid");
        ensureLimits(m, 1, M, "m isn't valid");
        ensureLimits(x, -MAX_VAL, MAX_VAL, "x=" + x + " isn't valid");
        inf.nextLine();
        for (int i = 0; i < n; ++i) {
            int ai = inf.nextInt();
            int bi = inf.nextInt();
            ...
            ensureLimits(ai, -MAX_VAL, MAX_VAL, "a[" + i + "]=" + ai + " isn't valid");
            ensureLimits(bi, -MAX_VAL, MAX_VAL, "b[" + i + "]=" + bi + " isn't valid");
            ensure(ai != bi, "a[" + i + "]=b[" + i + "]=" + ai);
            ensureLimits(vi, 1, MAX_VAL, "v[" + i + "]=" + vi + "is't valid");
            inf.nextLine();
        }
        for (int i = 0; i < m; ++i) {
            int t = inf.nextInt();
            ensureLimits(t, 0, MAX_VAL, "t[" + i + "]=" + t + " is't valid");
        }
        inf.nextLine();
        inf.close();
    }
