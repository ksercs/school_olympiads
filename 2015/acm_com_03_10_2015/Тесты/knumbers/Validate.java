...
public class Validate {
    private final static int MAXN = 1_000_000;
    private final static int MAXK = 10;

    public void run() {
        StrictScanner inf = new StrictScanner(System.in);
        int k = inf.nextInt();
        ensureLimits(k, 1, MAXK, "Wrong K -");
        inf.nextLine();
        String s = inf.next();
        inf.nextLine();
        ensureLimits(s.length(), 1, MAXN, "Wrong length of number -");
        ensure(s.charAt(0) != '0', "Number in input starts with zero");
        for (int i = 0; i < s.length(); i++) {
            ensure('0' <= s.charAt(i) && s.charAt(i) <= '9', "Symbol #" + (i + 1) + " is not a digit");
        }
        inf.close();
    }
