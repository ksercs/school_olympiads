...
public class Validate {
    public static final int MAX_SUM = 200000;
    ...
    public void run() {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            inf.nextLine();
            sum += s.length();
            ensureLimits(s.length(), 1, MAX_SUM, "Wrong length of string #" + i);
            for (int j = 0; j < s.length(); ++j)
                ensure('a' <= s.charAt(j) && s.charAt(j) <= 'z', "Incorrect character " + s.charAt(j) + " in string #" + i);
        }
        ensureLimits(sum, 1, MAX_SUM, "Too long total length");
        inf.close();
    }
