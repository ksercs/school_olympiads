...
public class Validate {
    ...
    private final static int MAXK = 100000;
    ...
    public void run() {
        ...
        int k = inf.nextInt();
        inf.nextLine();
        ensureLimits(n, 1, MAXN, "Wrong N -");
        ensureLimits(m, 0, n, "Wrong M -");
        ensureLimits(k, 1, MAXK, "Wrong K -");
        String s = inf.next();
        ensure(s.length() == k, "String length != K");
        for (int i = 0; i < s.length(); ++i) 
            ensure(s.charAt(i) == 'L' || s.charAt(i) == 'R' || s.charAt(i) == 'A', "Bad character: " + s.charAt(i));
        inf.nextLine();
        inf.close();
    }
